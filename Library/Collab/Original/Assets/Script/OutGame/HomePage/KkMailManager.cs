...
public class KkMailManager : MonoBehaviour
{
    ...
    public void KkMailInitialize()
    {
        ...
        for(int i = 0; i < mails.Count; i++)
        {
            ...
            if (isObtained)
            {
                Texture2D texture = Resources.Load("Textures/T_2_letter_open_") as Texture2D;
                NewMail.transform.GetChild(0).gameObject.GetComponent<Image>().sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f));
            }
            else
            {
                Texture2D texture = Resources.Load("Textures/T_1_letter_") as Texture2D;
                NewMail.transform.GetChild(0).gameObject.GetComponent<Image>().sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f));
            }
    ...
    private void GetMailDetail(KkMail mail)
    {
        ...
        if (mail.GetAttachment() != "")
        {
            ...
            for (int i = 0; i < ats.Length; i++)
            {
                ...
                Texture2D texture = Resources.Load("Textures/Props/"+atName) as Texture2D;
                NewAttachment.transform.gameObject.GetComponent<Image>().sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f));
                if(atName == "Hair" || atName == "NikeCoin" || atName == "Credit" || atName == "Exp")
                {
        ...
        else
        {
            DetailGetAllBtn.GetComponent<Button>().onClick.AddListener(delegate() 
            {
                ...
                UserController.GetUserControllerInstance().UpdateUserInfo(1);
            });
        }
    }
