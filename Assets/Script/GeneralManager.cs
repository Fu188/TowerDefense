...
public class GeneralManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        GenerateKkMails(user.GetUserId());
        GenerateEnemy(user.GetManual());
    }

    

    public List<Card> cards = new List<Card>();
    public List<Assets.Scripts.EnemyInfo> enemyInfos = new List<Assets.Scripts.EnemyInfo>();
    ...
    void GenerateEnemy(string haveEnemies)
    {
        string newEnemies = Resources.Load("Content/Enemy").ToString();
        print(haveEnemies);
        print(newEnemies);
        JArray jaHave = (JArray)JsonConvert.DeserializeObject(haveEnemies);
        JArray jaNew = (JArray)JsonConvert.DeserializeObject(newEnemies);
        int jaHaveCount = 0;
        print(enemyInfos);
        if (jaHave != null)
        {
            ...
            foreach (JToken j in jaHave)
            {
                EnemyInfo enemy = JsonConvert.DeserializeObject<EnemyInfo>(j.ToString());
                enemyInfos.Add(enemy);
            }
        }
        if (jaNew.Count > jaHaveCount)
        {
            for (int i = jaHaveCount; i < jaNew.Count; i++)
            {
                EnemyInfo enemy = JsonConvert.DeserializeObject<EnemyInfo>(jaNew[i].ToString());
                enemyInfos.Add(enemy);
            }
        }
        print(enemyInfos);
