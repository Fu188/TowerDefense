...
public class GameManager : MonoBehaviour, IPunObservable
{
    ...
    private void CreateLevelFromXML()
    {
        ...
        {
            GameObject go;
            //go = GameNetworkManager.KkInstantiate(PathPrefab, attr[0], Quaternion.Euler(attr[1]));
            go= Instantiate(PathPrefab, attr[0], Quaternion.Euler(attr[1])) as GameObject;
        ...
        GameObject tower;
        //tower = GameNetworkManager.KkInstantiate(keyPrefab, levelStuffFromXML.Tower[0], Quaternion.Euler(levelStuffFromXML.Tower[1]));
        tower = Instantiate(keyPrefab, levelStuffFromXML.Tower[0], Quaternion.Euler(levelStuffFromXML.Tower[1])) as GameObject;
        tower.transform.SetParent(otherObjectList.transform);
        GameObject EnemyGenerator;
        //EnemyGenerator = GameNetworkManager.KkInstantiate(EnemyGeneratorPrefab, levelStuffFromXML.EnemyGenerator[0], Quaternion.Euler(levelStuffFromXML.EnemyGenerator[1]));
        EnemyGenerator = Instantiate(EnemyGeneratorPrefab, levelStuffFromXML.EnemyGenerator[0], Quaternion.Euler(levelStuffFromXML.EnemyGenerator[1])) as GameObject;
    ...
    IEnumerator NextRound()
    {
        ...
        for (int i = 0; i < NoOfEnemies; i++)
        {
            ...
            GameObject enemy;
            enemy = GameNetworkManager.KkInstantiate(EnemyPrefab, Waypoints[0].position, Quaternion.Euler(levelStuffFromXML.Enemy));
            //enemy =  Instantiate(EnemyPrefab, Waypoints[0].position, Quaternion.Euler(levelStuffFromXML.Enemy)) as GameObject;
    ...
    void Update()
    {
        int count = Peoples.Where(x => x != null).Count();
        if (count != 0)
        {
           ...
           < Vector3.Distance(next.transform.position, peopleTransform.position) ? current : next);
            if (Vector3.Distance(targetPeople.transform.position, peopleTransform.position) < Constants.MinDistanceToDestroyPeople)
            {
                ...
                {
                    //TargetEffect = GameNetworkManager.KkInstantiate(TargetEffectPrefab, targetPeople.transform.position + new Vector3(0, 0.8f, 0), targetPeople.transform.rotation);
    ...
    private void DestroyExistingEnemiesAndCoins()
    {
        ...
        foreach (var item in Enemies)
        {
            if (item != null)
            {
                GameNetworkManager.KkDestroy(item.gameObject);
            }
        }
        //get all the coins
    ...
    public void createPeople(int peopleType)
    {
        ...
        {
            GameNetworkManager.KkDestroy(go);
        }
        else
        {
    ...
    public void removePeople()
    {
        ...
        if (count != 0)
        {
            ...
            if (Vector3.Distance(targetPeople.transform.position, peopleTransform.position) < Constants.MinDistanceToDestroyPeople)
            {
                ...
                GameNetworkManager.KkDestroy(TargetEffect);
            }
        }
    ...
    public void useProp(int proIndex)
    {
        switch (proIndex)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                CoinSpawner.GetCoinSpawnInstance().MulCoinSpawnSpeed(5f, 30f);
                break;
            case 3:
                foreach (var item in Enemies)
                {
            ...
            case 4:
                ...
                break;
            case 5:
                KKHeroManager.GetKKHeroManagerInstance().BiggerHero(3f, 30f);
                break;
            case 6:
                Lives++;
                break;
            ...
            case 9:
                print("ready");
                KKHeroManager.GetKKHeroManagerInstance().BackHome();
                break;
        }
    }
       ...
       /* if (stream.IsWriting)
        {
            ...
            stream.SendNext(FinalRoundFinished);
        }
        else
        {
            this.Lives = (int)stream.ReceiveNext();
            //this.CurrentGameState = (GameState)stream.ReceiveNext();
            this.currentRoundIndex = (int)stream.ReceiveNext();
            this.FinalRoundFinished = (bool)stream.ReceiveNext();
        }*/
    }
}
